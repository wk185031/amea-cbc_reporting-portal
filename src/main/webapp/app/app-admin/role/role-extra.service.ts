...
export class RoleExtraService {
    ...
    private resourceUrlPagination = SERVER_API_URL + 'api/role-extras-pagination';
    ...
    findAllRoleExtrasWithPagination(req?: any): Observable<HttpResponse<RoleExtra[]>> {
        const options = createRequestOption(req);
        return this.http.get<RoleExtra[]>(this.resourceUrlPagination, { params: options, observe: 'response' })
            .map((res: HttpResponse<RoleExtra[]>) => this.convertArrayResponse(res));
    }
