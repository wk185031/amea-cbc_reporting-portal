...
select
	...
	'''' AS "JOINT ACC OPERATION FLAG"
from
	issuance_cash_card_request@{DB_LINK_DCMS} ccr
	inner join issuance_cash_card@{DB_LINK_DCMS} csh on csh.csh_id = ccr.ccr_csh_id
	inner join issuance_client@{DB_LINK_DCMS} clt on clt.clt_id = ccr.ccr_clt_id
	inner join issuance_bank_account@{DB_LINK_DCMS} bac on bac.bac_clt_id = clt.clt_id
	inner join card_program_setup@{DB_LINK_DCMS} prs on prs.prs_id = ccr.ccr_prs_id
	inner join master_status@{DB_LINK_DCMS} sts on sts_id = ccr.ccr_sts_id
	where ccr.ccr_ins_id = {Iss_Id}) tb
order by
	tb.cif, tb."ATM CARD NUMBER"
	');
	
	update REPORT_DEFINITION set RED_BODY_FIELDS = i_BODY_FIELD,RED_BODY_QUERY = i_BODY_QUERY where RED_NAME = 'Finacle 360';
