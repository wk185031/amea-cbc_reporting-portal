...
SELECT
	...
	WHERE
      (TXN.TRL_TQU_ID = ''F'' OR (TXN.TRL_TQU_ID = ''A'' AND (TRL_TSC_CODE IN (90, 129, 131, 142, 143) OR TRL_TSC_CODE = 126 AND 
		ATA.ATA_TXN_TYPE IN (''EBK Pin Registration'', ''MBK Pin Registration''))) OR (TXN.TRL_TQU_ID = ''R'' AND TXN.TRL_ACTION_RESPONSE_CODE = 0))
...
SELECT
      ...
      TXN.TRL_DATETIME_LOCAL_TXN "TIME",
      CASE
		WHEN (TXNC.TRL_IS_INTER_ENTITY = 1 OR (CBA.CBA_MNEM IS NOT NULL AND CBA.CBA_MNEM != {V_Deo_Name})) AND TXN.TRL_TQU_ID = ''R'' AND CTRI.CTR_REV_MNEM IS NOT NULL THEN CTRI.CTR_REV_MNEM 
		WHEN (TXNC.TRL_IS_INTER_ENTITY = 1 OR (CBA.CBA_MNEM IS NOT NULL AND CBA.CBA_MNEM != {V_Deo_Name})) AND TXN.TRL_TQU_ID = ''F'' AND CTRI.CTR_MNEM IS NOT NULL THEN CTRI.CTR_MNEM
...
SELECT
	...
	WHERE
      (TXN.TRL_TQU_ID IN (''F'', ''C'') OR (TXN.TRL_TQU_ID = ''R'' AND TXN.TRL_ACTION_RESPONSE_CODE = 0))
	  AND TXN.TRL_FRD_REV_INST_ID IS NOT NULL
      AND TXN.TRL_TSC_CODE IN (40, 43, 42, 44, 45, 46, 47, 48, 49, 51, 52, 252)
