...
SELECT
      ...
      CASE 
        ...
        WHEN TXN.TRL_TSC_CODE = 52 AND TXN.TRL_FRD_REV_INST_ID = ''0010'' THEN ''GLOBE''
		WHEN TXN.TRL_TSC_CODE = 52 AND TXN.TRL_FRD_REV_INST_ID = ''0020'' THEN ''SMART''
		WHEN TXN.TRL_TSC_CODE = 52 AND TXN.TRL_FRD_REV_INST_ID = ''0030'' THEN ''SUN''
...
SELECT
      ...
      CASE 
		WHEN TXN.TRL_TSC_CODE IN (50, 250) THEN CBL.CBL_MNEM
        WHEN LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = ''0000008882'' AND SUBSTR(TXN.TRL_ACCOUNT_2_ACN_ID, 1, 5) = ''00010'' THEN ''GLOBE''
		WHEN LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = ''0000008882'' AND SUBSTR(TXN.TRL_ACCOUNT_2_ACN_ID, 1, 5) = ''00020'' THEN ''SMART''
        WHEN LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = ''0000008882'' AND SUBSTR(TXN.TRL_ACCOUNT_2_ACN_ID, 1, 5) = ''00030'' THEN ''SUN''
        WHEN TXN.TRL_FRD_REV_INST_ID IS NOT NULL THEN CBA_REV.CBA_MNEM
		WHEN TXN.TRL_ACCOUNT_TYPE_2_ATP_ID = 10 THEN ''SA'' 
		WHEN TXN.TRL_ACCOUNT_TYPE_2_ATP_ID = 20 THEN ''CA'' 
		ELSE '''' END AS "TO ACC TYPE",
	...
	WHERE
      ...
      AND TXN.TRL_TSC_CODE IN (1, 2, 22, 42, 43, 45, 46, 47, 48, 49)
      AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_Iss_Name} 
	  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
      AND {Branch_Code}
      AND {Terminal}
