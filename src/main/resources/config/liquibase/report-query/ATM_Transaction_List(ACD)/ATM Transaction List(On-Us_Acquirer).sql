...
-- CBCAXUPISSLOG-1167	25-MAR-2022		WY		Get branch code, name based on txn branch code instead of ATM station branch confoguration
...
SELECT
	...
	FROM
      ...
      LEFT JOIN CBC_TRAN_CODE CTRD ON TXN.TRL_TSC_CODE = CTRD.CTR_CODE AND CTRD.CTR_CHANNEL = (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''I-CDM'' ELSE ''I-'' || TXNC.TRL_ORIGIN_CHANNEL END) AND CTRD.CTR_DEBIT_CREDIT = ''DEBIT''
	  LEFT JOIN CBC_BIN CBI ON CBI.CBI_ID = (SELECT MIN(CBI_ID) FROM CBC_BIN WHERE CBI_BIN = TXNC.TRL_CARD_BIN)
      LEFT JOIN CBC_BANK CBA ON CBI.CBI_CBA_ID = CBA.CBA_ID      
      LEFT JOIN AUTH_RESULT_CODE ARC ON TXN.TRL_ACTION_RESPONSE_CODE = ARC.ARC_CODE
      LEFT JOIN ATM_STATIONS AST ON TXN.TRL_CARD_ACPT_TERMINAL_IDENT = AST.AST_TERMINAL_ID
...
SELECT
	...
	FROM
	  ...
	  LEFT JOIN CBC_BANK CBA_REV ON LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = LPAD(CBA_REV.CBA_CODE, 10, ''0'')
	WHERE
      (TXN.TRL_TQU_ID = ''F''  OR (TXN.TRL_TQU_ID = ''A'' AND TRL_TSC_CODE IN (90, 126, 127) OR (TXN.TRL_TQU_ID = ''R'' AND TXN.TRL_ACTION_RESPONSE_CODE = 0)))
      AND TXN.TRL_FRD_REV_INST_ID IS NULL
