...
SELECT
	...
	SELECT DISTINCT
      ...
      CASE WHEN TXN.TRL_TQU_ID != ''R'' AND TXN.TRL_ACTION_RESPONSE_CODE = 0 THEN 1 ELSE 0 END AS "GOOD TRANS",
      CASE WHEN TXN.TRL_TQU_ID != ''R'' AND TXN.TRL_ACTION_RESPONSE_CODE > 0 THEN 1 ELSE 0 END AS "BAD TRANS",
      CASE WHEN TXN.TRL_TQU_ID != ''R'' THEN 1 ELSE 0 END AS "TOTAL TRANS",
      ...
      CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_TSC_CODE IN (50, 51, 133, 146, 246, 250) THEN TXN.TRL_AMT_TXN ELSE 0 END AS "BILL/INS PAYMENTS",
    ...
    SELECT DISTINCT
      CASE 
		WHEN TXN.TRL_TSC_CODE = 1 AND TXN.TRL_TQU_ID = ''R'' AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = ''0000008882'' THEN (SELECT CTR_REV_MNEM FROM CBC_TRAN_CODE WHERE CTR_CODE = 52 AND CTR_DEBIT_CREDIT = ''DEBIT'' AND CTR_CHANNEL = CTR.CTR_CHANNEL)
        WHEN TXN.TRL_TSC_CODE = 1 AND TXN.TRL_TQU_ID = ''F'' AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = ''0000008882'' THEN (SELECT CTR_MNEM FROM CBC_TRAN_CODE WHERE CTR_CODE = 52 AND CTR_DEBIT_CREDIT = ''DEBIT'' AND CTR_CHANNEL = CTR.CTR_CHANNEL)
		WHEN TXN.TRL_TSC_CODE = 1 AND TXN.TRL_TQU_ID = ''R'' AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') != ''0000008882'' THEN (SELECT CTR_REV_MNEM FROM CBC_TRAN_CODE WHERE CTR_CODE = 44 AND CTR_DEBIT_CREDIT = ''DEBIT'' AND CTR_CHANNEL = CTR.CTR_CHANNEL)
        WHEN TXN.TRL_TSC_CODE = 1 AND TXN.TRL_TQU_ID = ''F'' AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') != ''0000008882'' THEN (SELECT CTR_MNEM FROM CBC_TRAN_CODE WHERE CTR_CODE = 44 AND CTR_DEBIT_CREDIT = ''DEBIT'' AND CTR_CHANNEL = CTR.CTR_CHANNEL)
		WHEN TXN.TRL_TQU_ID = ''R'' THEN CTR.CTR_REV_MNEM ELSE CTR.CTR_MNEM END AS "TRAN MNEM",
      CASE WHEN TXN.TRL_POST_COMPLETION_CODE is null AND TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_AMT_COMPLETED < TXN.TRL_AMT_TXN THEN 1 ELSE 0 END AS "SHORT DISP",
      CASE WHEN TXN.TRL_POST_COMPLETION_CODE is null AND TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_AMT_COMPLETED > TXN.TRL_AMT_TXN THEN 1 ELSE 0 END AS "OVER DISPENSE",
      CASE WHEN TXN.TRL_TQU_ID != ''R'' AND TXN.TRL_ACTION_RESPONSE_CODE = 0 THEN 1 ELSE 0 END AS "GOOD TRANS",
	...
	WHERE
      ...
      AND TXN.TRL_TSC_CODE IN (1, 42, 43, 45, 46, 47, 48, 49)
      AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_Iss_Name} 
	  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
      AND {Txn_Date}	
	)
	...
	WHERE
      ...
      AND TXN.TRL_CARD_ACPT_TERMINAL_IDENT != 12345
      AND ((TXN.TRL_ISS_NAME IS NULL AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') != {V_Iss_Name}) OR COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_IE_Iss_Name})
	  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
      AND {Txn_Date}
    UNION ALL
...
SELECT
	...
	SELECT DISTINCT
	  ...
	  CASE WHEN TXN.TRL_POST_COMPLETION_CODE = ''R'' AND TXN.TRL_TSC_CODE IN (50, 51, 133, 146, 246, 250) THEN TXN.TRL_AMT_TXN ELSE 0 END AS "BP REV",
	...
	WHERE
      (TXN.TRL_TQU_ID IN (''F'', ''R'') OR (TXN.TRL_TQU_ID = ''A'' AND TRL_TSC_CODE IN (90, 126, 129, 131, 142, 143)))
      AND TXN.TRL_FRD_REV_INST_ID IS NULL 
      AND TXN.TRL_TSC_CODE NOT IN (2, 22, 42, 43, 44, 45, 46, 47, 48, 49, 51, 52, 252)
      AND TXN.TRL_CARD_ACPT_TERMINAL_IDENT != 12345
      AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_Iss_Name} 
	  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
      AND {Txn_Date}
	...
	WHERE
      TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'')
      AND TXN.TRL_FRD_REV_INST_ID IS NOT NULL
      AND TXN.TRL_TSC_CODE IN (1, 42, 43, 45, 46, 47, 48, 49, 51)
      AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_Iss_Name} 
	  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
      AND {Txn_Date}
	)
	...
	WHERE
      TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'')
      AND TXN.TRL_FRD_REV_INST_ID IS NULL 
      AND TXN.TRL_TSC_CODE NOT IN (2, 22, 40, 42, 43, 44, 45, 46, 47, 48, 49, 51, 52, 252)
      AND TXN.TRL_CARD_ACPT_TERMINAL_IDENT != 12345
      AND ((TXN.TRL_ISS_NAME IS NULL AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') != {V_Iss_Name}) OR COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_IE_Iss_Name})
	  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
      AND {Txn_Date}
