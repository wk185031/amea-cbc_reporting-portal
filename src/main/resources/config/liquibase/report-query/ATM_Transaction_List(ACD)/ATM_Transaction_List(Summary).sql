...
SELECT
	...
	SELECT DISTINCT
      ...
      CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_TQU_ID IN (''F'', ''A'') AND TXN.TRL_POST_COMPLETION_CODE IS NULL) THEN 1 ELSE NULL END AS "GOOD TRANS",
 	  CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE > 0 OR (TXN.TRL_ACTION_RESPONSE_CODE = 0 AND (TXN.TRL_TQU_ID = ''R'' OR TXN.TRL_POST_COMPLETION_CODE IS NOT NULL))) THEN 1 ELSE NULL END AS "BAD TRANS",
  	  ...
  	  CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_TQU_ID IN (''F'', ''A'') AND TXN.TRL_POST_COMPLETION_CODE IS NULL AND TXN.TRL_TSC_CODE IN (1, 128, 142, 143)) THEN TXN.TRL_AMT_TXN ELSE 0 END AS "CASH DISPENSED",
	...
	SELECT DISTINCT
      ...
      CASE WHEN (TXNC.TRL_IS_INTER_ENTITY = 1 OR (CBA.CBA_MNEM IS NOT NULL AND CBA.CBA_MNEM != {V_Deo_Name})) AND TXN.TRL_TQU_ID = ''R'' AND CTRI.CTR_REV_MNEM IS NOT NULL THEN CTRI.CTR_REV_MNEM 
