...
-- CBCAXUPISSLOG-961	15-OCT-2021		AM		Fix wrong total count
...
WHERE
  (TXN.TRL_TQU_ID IN (''F'', ''R'') OR (TXN.TRL_TQU_ID = ''A'' AND TXNC.TRL_ORIGIN_CHANNEL=''OTC'') OR (TXN.TRL_TQU_ID = ''A'' AND (TRL_TSC_CODE IN (90, 126, 127, 129, 131, 142, 143)))) 
  AND TXN.TRL_FRD_REV_INST_ID IS NULL 
  AND TXN.TRL_TSC_CODE NOT IN (42, 44, 45, 46, 47, 48, 49, 52, 252)
  AND (TXN.TRL_CARD_ACPT_TERMINAL_IDENT != ''12345'' OR TXNC.TRL_ORIGIN_CHANNEL=''OTC'')
...
FROM TRANSACTION_LOG TXN JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID=TXNC.TRL_ID
  LEFT JOIN CBC_TRAN_CODE CTR ON TXN.TRL_TSC_CODE = CTR.CTR_CODE AND CTR.CTR_CHANNEL = (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''CDM'' ELSE TXNC.TRL_ORIGIN_CHANNEL END) AND CTR.CTR_CODE NOT IN (48, 49) AND CTR.CTR_DEBIT_CREDIT IN (''DEBIT'', ''CREDIT'')
   LEFT JOIN CBC_TRAN_CODE CTRD ON TXN.TRL_TSC_CODE = CTRD.CTR_CODE AND (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''I-CDM'' ELSE ''I-'' || TXNC.TRL_ORIGIN_CHANNEL END) = CTRD.CTR_CHANNEL AND CTRD.CTR_DEBIT_CREDIT = ''DEBIT''
  LEFT JOIN CBC_BIN CBI ON CBI.CBI_ID = (SELECT MIN(CBI_ID) FROM CBC_BIN WHERE CBI_BIN = TXNC.TRL_CARD_BIN)
  LEFT JOIN CBC_BANK CBA ON CBI.CBI_CBA_ID = CBA.CBA_ID    
WHERE
  TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'')
  AND TXN.TRL_FRD_REV_INST_ID IS NOT NULL
  AND TXN.TRL_TSC_CODE IN (42, 43, 44, 45, 46, 47, 48, 49, 51, 52, 252 , 251)
  AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_Iss_Name} 
  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
  AND {Txn_Date}	
)
...
WHERE
  (TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'') OR (TXN.TRL_TQU_ID = ''A'' AND TRL_TSC_CODE IN (90, 126, 127)))
  AND TXN.TRL_FRD_REV_INST_ID IS NULL 
  AND TXN.TRL_TSC_CODE NOT IN (40, 42, 44, 45, 46, 47, 48, 49, 51, 52, 252 , 251)
...
WHERE
  ...
  AND TXN.TRL_TSC_CODE IN (40, 42, 43, 44, 45, 46, 47, 48, 49, 51, 52, 252 , 251)
...
FROM TRANSACTION_LOG TXN JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID=TXNC.TRL_ID
  ...
  LEFT JOIN CBC_TRAN_CODE CTRI ON TXN.TRL_TSC_CODE = CTRI.CTR_CODE AND CTRI.CTR_CHANNEL = (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''I-CDM'' ELSE ''I-'' || TXNC.TRL_ORIGIN_CHANNEL END) AND CTRI.CTR_DEBIT_CREDIT = ''DEBIT''
  LEFT JOIN CBC_BIN CBI ON CBI.CBI_ID = (SELECT MIN(CBI_ID) FROM CBC_BIN WHERE CBI_BIN = TXNC.TRL_CARD_BIN)
  LEFT JOIN CBC_BANK CBA ON CBI.CBI_CBA_ID = CBA.CBA_ID
WHERE
  TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'')
...
FROM TRANSACTION_LOG TXN JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID=TXNC.TRL_ID
  LEFT JOIN CBC_TRAN_CODE CTR ON TXN.TRL_TSC_CODE = CTR.CTR_CODE AND CTR.CTR_CHANNEL = ''BNT''
  LEFT JOIN CBC_TRAN_CODE CTRI ON TXN.TRL_TSC_CODE = CTRI.CTR_CODE AND CTRI.CTR_CHANNEL = (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''I-CDM'' ELSE ''I-'' || TXNC.TRL_ORIGIN_CHANNEL END) AND CTRI.CTR_DEBIT_CREDIT = ''CREDIT''
  LEFT JOIN CBC_BIN CBI ON CBI.CBI_ID = (SELECT MIN(CBI_ID) FROM CBC_BIN WHERE CBI_BIN = TXNC.TRL_CARD_BIN)
  LEFT JOIN CBC_BANK CBA ON CBI.CBI_CBA_ID = CBA.CBA_ID
WHERE
  TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'')
...
FROM TRANSACTION_LOG TXN JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID=TXNC.TRL_ID
  LEFT JOIN CBC_TRAN_CODE CTR ON TXN.TRL_TSC_CODE = CTR.CTR_CODE AND CTR.CTR_CHANNEL = (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''CDM'' ELSE TXNC.TRL_ORIGIN_CHANNEL END) AND CTR.CTR_CODE NOT IN (48, 49) AND CTR.CTR_DEBIT_CREDIT IN (''DEBIT'', ''CREDIT'')
  LEFT JOIN CBC_TRAN_CODE CTRD ON TXN.TRL_TSC_CODE = CTRD.CTR_CODE AND (CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BRM'' THEN ''I-CDM'' ELSE ''I-'' || TXNC.TRL_ORIGIN_CHANNEL END) = CTRD.CTR_CHANNEL AND CTR.CTR_DEBIT_CREDIT = ''DEBIT''
  LEFT JOIN CBC_BIN CBI ON CBI.CBI_ID = (SELECT MIN(CBI_ID) FROM CBC_BIN WHERE CBI_BIN = TXNC.TRL_CARD_BIN)
  LEFT JOIN CBC_BANK CBA ON CBI.CBI_CBA_ID = CBA.CBA_ID
WHERE
  TXN.TRL_TQU_ID IN (''F'', ''R'', ''C'')
  AND TXN.TRL_FRD_REV_INST_ID IS NOT NULL
  AND TXN.TRL_TSC_CODE IN (42, 43, 44, 45, 46, 47, 48, 49, 51, 52 , 251)
  AND COALESCE(CBA.CBA_MNEM, TXN.TRL_ISS_NAME, '''') = {V_Iss_Name} 
  AND (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id})
  AND {Txn_Date})
START ACQ SELECT
