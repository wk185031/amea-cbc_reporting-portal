...
DECLARE
    ...
    i_PROCESSING_CLASS VARCHAR2(200) := 'my.com.mandrill.base.reporting.atmTransactionListsBranch.transactionSummaryGrandTotal.TxnSummaryCashCardProcessor';
...
FROM (
  ...
  CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE > 0 OR TXN.TRL_TQU_ID = ''R'') THEN TXN.TRL_AMT_TXN ELSE NULL END AS "NO DISP",
...
FROM (
  SELECT
      ...
      CASE WHEN TXN.TRL_TQU_ID = ''R'' THEN CTR.CTR_REV_MNEM ELSE CTR.CTR_MNEM END AS "TRAN MNEM",
  CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_TQU_ID = ''F'') THEN 1 ELSE NULL END AS "GOOD TRANS",
  CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE > 0 AND TXN.TRL_TQU_ID = ''F'') THEN 1 ELSE NULL END AS "BAD TRANS",
  CASE WHEN TXN.TRL_TQU_ID = ''R'' THEN 1 ELSE NULL END AS "NO DISP",
  CASE WHEN (TXN.TRL_ACTION_RESPONSE_CODE = 0 OR TXN.TRL_TQU_ID = ''R'') THEN TXN.TRL_AMT_TXN ELSE 0 END AS "CASH DISPENSED"	
FROM
      TRANSACTION_LOG TXN
      JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID=TXNC.TRL_ID
      JOIN CBC_TRAN_CODE CTR ON TXN.TRL_TSC_CODE = CTR.CTR_CODE AND TXNC.TRL_ORIGIN_CHANNEL = CTR.CTR_CHANNEL
