...
BEGIN 
	...
	SELECT
      ...
      CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 AND CTR.CTR_MNEM IN (''CTW'',''CFC'',''MWD'',''MWP'') THEN TXN.TRL_AMT_TXN ELSE 0 END AS "DISPENSED",     
      CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 AND CTR.CTR_MNEM = ''CSM'' THEN TXN.TRL_AMT_TXN ELSE 0 END AS "DEPOSITS",
      CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 AND CTR.CTR_MNEM IN (''CBY'',''MZD'',''MED'') THEN TXN.TRL_AMT_TXN ELSE 0 END AS "BILL/INS PAYMENTS",
      CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 AND CTR.CTR_MNEM = ''CLP'' THEN TXN.TRL_AMT_TXN ELSE 0 END AS "LOAN",
      CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 AND CTR.CTR_MNEM IN (''MXD'',''MID'',''MTD'') THEN TXN.TRL_AMT_TXN ELSE 0 END AS "TRANSFERS"
	FROM
      TRANSACTION_LOG TXN
