...
SELECT
  ...
  CTR.CTR_MNEM "TRANSACTION TYPE",
  COUNT(CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 THEN 1 ELSE NULL END) AS "SUCC TXN COUNT",
  SUM(CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE = 0 THEN TXN.TRL_AMT_TXN ELSE 0 END) "SUCC TXN AMT",
  COUNT(CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE != 0 THEN 1 ELSE NULL END) AS "UNSUCC TXN COUNT",
  SUM(CASE WHEN TXN.TRL_ACTION_RESPONSE_CODE != 0 THEN TXN.TRL_AMT_TXN ELSE 0 END) "UNSUCC TXN AMT",
...
FROM
  ...
  LEFT JOIN CBC_TRAN_CODE CTR ON 
      ...
      AND CTR.CTR_CHANNEL = CASE WHEN (TXN.TRL_TSC_CODE = 48 OR TXN.TRL_DEO_NAME = {V_IE_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, 0) = {V_IE_Acqr_Inst_Id} OR LPAD(TXN.TRL_FRD_REV_INST_ID, 10, 0) = {V_IE_Acqr_Inst_Id}) THEN CONCAT(''I-'', TXNC.TRL_ORIGIN_CHANNEL) ELSE TXNC.TRL_ORIGIN_CHANNEL END
