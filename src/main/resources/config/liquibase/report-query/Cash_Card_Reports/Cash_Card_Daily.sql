...
SELECT
    ...
    CASE WHEN (TXN.TRL_TSC_CODE != 22 AND TXN.TRL_TSC_CODE != 21 AND TXN.TRL_TQU_ID = ''F'') THEN (TXN.TRL_AMT_TXN + COALESCE(TXN.TRL_ACQ_CHARGE_AMT, TXN.TRL_ISS_CHARGE_AMT, 0)) ELSE 0 END "DR AMOUNT",
    ...
    CASE WHEN TXN.TRL_TSC_CODE IN (42, 43, 44, 45, 48, 52) AND TXN.TRL_TQU_ID = ''R'' THEN (TRL_ACCOUNT_1_MAX_AVAILABLE  + TXN.TRL_AMT_TXN + COALESCE(TXN.TRL_ACQ_CHARGE_AMT, TXN.TRL_ISS_CHARGE_AMT, 0)) 
      WHEN TXN.TRL_TSC_CODE IN (42, 43, 44, 45, 48, 52) AND TXN.TRL_TQU_ID = ''F'' THEN (TRL_ACCOUNT_1_MAX_AVAILABLE  - TXN.TRL_AMT_TXN - COALESCE(TXN.TRL_ACQ_CHARGE_AMT, TXN.TRL_ISS_CHARGE_AMT, 0)) 
...
SELECT
    ...
    CASE WHEN TXN.TRL_TQU_ID = ''F'' THEN TXN.TRL_AMT_TXN ELSE 0 END "CR AMOUNT",
