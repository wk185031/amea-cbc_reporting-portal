...
BEGIN 	 
  ...
  i_BODY_QUERY := TO_CLOB('
	select 
		...
		sum(CASE WHEN TXN.TRL_TSC_CODE IN (42, 43, 44, 45, 48, 52) AND TXN.TRL_TQU_ID = ''R'' THEN (TRL_ACCOUNT_1_MAX_AVAILABLE  + TXN.TRL_AMT_TXN + COALESCE(TXN.TRL_ACQ_CHARGE_AMT, TXN.TRL_ISS_CHARGE_AMT, 0)) 
	...
	where 
	  ...
	  AND {Txn_Date}
	group by 
  	  TXNC.TRL_CARD_BRANCH, BRC.BRC_NAME,TXNC.TRL_ORIGIN_CHANNEL,TXNC.TRL_IS_CORPORATE_CARD
	order by TXNC.TRL_CARD_BRANCH, BRC.BRC_NAME,"TRANSACTION GROUP" desc,TXNC.TRL_ORIGIN_CHANNEL
  '); 
