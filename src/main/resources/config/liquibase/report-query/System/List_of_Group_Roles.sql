...
BEGIN 
	  ...
	  i_BODY_QUERY := TO_CLOB('select
		JHI_USER.LOGIN as User_ID,
		concat (concat(JHI_USER.FIRST_NAME, '' ''), JHI_USER.LAST_NAME) as Username,
		BRANCH.BRC_NAME as Branch,
		ROLE_EXTRA.NAME as Role_ID,
		ROLE_EXTRA.DESCRIPTION as Role_Name,
		USER_EXTRA.LAST_LOGIN_TS as Last_Login,
		JHI_USER.CREATED_DATE as Created_Date,
		JHI_USER.DEACTIVATE_REASON as Last_Status,
		case when JHI_USER.ACTIVATED = 1 then ''ACTIVE'' else ''INACTIVE'' end as Status,
		JHI_USER.DEACTIVATE_DATE as Status_Change_Date,
		JHI_USER.RESET_DATE as Password_Change_Date
		from JHI_USER
		join USER_EXTRA on JHI_USER.ID = USER_EXTRA.USER_ID
		join USER_EXTRA_BRANCHES on USER_EXTRA_BRANCHES.USER_EXTRA_ID = USER_EXTRA.ID
		join BRANCH on BRANCH.BRC_CODE = USER_EXTRA_BRANCHES.BRANCH_ID
		join USER_EXTRA_ROLES on USER_EXTRA_ROLES.USER_EXTRAS_ID = USER_EXTRA.ID
		join ROLE_EXTRA on USER_EXTRA_ROLES.ROLES_ID = ROLE_EXTRA.ID');
