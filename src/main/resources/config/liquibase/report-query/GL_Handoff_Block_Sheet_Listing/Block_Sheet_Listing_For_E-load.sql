...
-- Eload Reports Specification	02-JUL-2021		GS		Modify and restructure the logic of GL Blocksheet Eload Reports
...
SELECT
      ...
      CASE WHEN (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id}) THEN TXN.TRL_DEST_STAN ELSE COALESCE(TXN.TRL_STAN, TXN.TRL_DEST_STAN, NULL) END AS "CODE",
...
GROUP BY
      GLA.GLA_NUMBER,
      GLA.GLA_NAME,
      CASE WHEN (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id}) THEN TXN.TRL_DEST_STAN ELSE COALESCE(TXN.TRL_STAN, TXN.TRL_DEST_STAN, NULL),
      TXN.TRL_AMT_TXN,
...
ORDER BY
      CASE WHEN (TXN.TRL_DEO_NAME = {V_Deo_Name} OR LPAD(TXN.TRL_ACQR_INST_ID, 10, ''0'') = {V_Acqr_Inst_Id}) THEN TXN.TRL_DEST_STAN ELSE COALESCE(TXN.TRL_STAN, TXN.TRL_DEST_STAN, NULL) END ASC,
...
END
	...
	WHERE RED_NAME = 'Block Sheet Listing For E-load' AND RED_INS_ID = 22;
	...
	UPDATE REPORT_DEFINITION set 
		...
		RED_BODY_QUERY = i_BODY_QUERY,
		RED_TRAILER_QUERY = i_TRAILER_QUERY
	WHERE RED_NAME = 'Block Sheet Listing For E-load' AND RED_INS_ID = 2;
