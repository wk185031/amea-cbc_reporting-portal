...
FROM
	  ...
	  LEFT JOIN ACCOUNT_TYPE ATP ON ATP.ATP_ID = TXN.TRL_ACCOUNT_TYPE_2_ATP_ID
WHERE
      TXN.TRL_TQU_ID = ''F''
...
SELECT
      CASE WHEN GLE.GLE_DEBIT_DESCRIPTION = ''BANCNET SERVICE CHARGE'' THEN SUM(25.00) ELSE SUM(TXN.TRL_AMT_TXN) END AS "TOTAL DEBIT",
      0 AS "TOTAL CREDIT"
FROM
      TRANSACTION_LOG TXN
	  LEFT JOIN TRANSACTION_LOG_CUSTOM TLC ON TXN.TRL_ID = TLC.TRL_ID
...
START SELECT
      0 AS "TOTAL DEBIT",
      CASE WHEN GLE.GLE_CREDIT_DESCRIPTION = ''BANCNET SERVICE CHARGE'' THEN SUM(25.00) ELSE SUM(TXN.TRL_AMT_TXN) END AS "TOTAL CREDIT"
FROM
      TRANSACTION_LOG TXN
	  LEFT JOIN TRANSACTION_LOG_CUSTOM TLC ON TXN.TRL_ID = TLC.TRL_ID
      LEFT JOIN CBC_GL_ENTRY GLE ON TXN.TRL_TSC_CODE = GLE.GLE_TRAN_TYPE
