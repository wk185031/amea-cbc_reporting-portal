...
-- JIRA 917				13-SEPT-2021	WY		Extract Exception message based on message format
...
BEGIN 
  ...
  SELECT
      ...
      REGEXP_SUBSTR(AJL.AJL_EJ_DATA,''(\d){4}+\*[U|S]+\*[A-Z|a-z]+\*(\d){1,13},M-(\d){0,2},R-(\d){0,5}'')  "EXCEPTION"
	FROM
      ATM_STATIONS AST
      JOIN ATM_JOURNAL_LOG AJL ON AST.AST_ID = AJL.AJL_AST_ID
	  JOIN DEVICE_ESTATE_OWNER DEO ON AST.AST_DEO_ID = DEO.DEO_ID
    WHERE (AJL.AJL_EJ_DATA LIKE ''%*U*%'' OR AJL.AJL_EJ_DATA LIKE ''%*S*%'')
    ...
    SELECT
      ...
      AST.AST_ALO_LOCATION_ID "LOCATION",
      AJL.AJL_TIMESTAMP "DATE TIME",
      REGEXP_SUBSTR(AJL.AJL_EJ_DATA,''\*(\d){4}\*2\*[A-Z|1-9]+\*N'')  "EXCEPTION"
      FROM
      ATM_STATIONS AST
      JOIN ATM_JOURNAL_LOG AJL ON AST.AST_ID = AJL.AJL_AST_ID
	  JOIN DEVICE_ESTATE_OWNER DEO ON AST.AST_DEO_ID = DEO.DEO_ID
       WHERE AJL.AJL_EJ_DATA LIKE ''%*2*%''
       AND DEO.DEO_NAME = {V_Deo_Name}
	   AND {Txn_Date}
       )
