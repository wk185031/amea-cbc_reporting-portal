...
SELECT
	  ...
	  CASE WHEN TXN.TRL_ISS_NAME = {V_Iss_Name} AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = {V_IE_Recv_Inst_Id} AND TXN.TRL_PAN != ''FD4CD08B482F7961EA66FBEA7C7583B541F82B3E6A915B4D7E9191D8FC5FB971'' THEN 1 END AS "ONLINE RETAIL COUNT",
      CASE WHEN LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = {V_IE_Recv_Inst_Id} AND TXN.TRL_PAN = ''FD4CD08B482F7961EA66FBEA7C7583B541F82B3E6A915B4D7E9191D8FC5FB971'' THEN 1  END AS "ONLINE CORPORATE COUNT",
      CASE WHEN TXN.TRL_ISS_NAME = {V_Iss_Name} AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = {V_IE_Recv_Inst_Id} AND BRC.BRC_CODE IS NOT NULL THEN 1
               ...
               WHEN TXN.TRL_ISS_NAME = {V_IE_Iss_Name} AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') NOT IN ({V_Recv_Inst_Id}, {V_IE_Recv_Inst_Id}) THEN 1
      END AS "ATM COUNT",
