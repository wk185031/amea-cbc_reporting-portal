...
-- CBS header/body/trailer fields
	...
	i_BODY_QUERY_2 := TO_CLOB('Union All
		...
		SELECT DISTINCT
			...
			CASE WHEN IDCR.DCR_STS_ID IN (68,69) THEN IDCR.DCR_NUMBER ELSE JSON_VALUE(IDCR.DCR_CIF_RESPONSE, ''$.bankAccountCollection.bacAccountNumber'') END AS ACC_NO,
			INS.INS_CODE AS INSTITUTION_ID,
			IC.CRD_KEY_ROTATION_NUMBER ROTATION_NUMBER,
			STF1.STF_LOGIN_NAME As Maker,
			STF2.STF_LOGIN_NAME As Checker,
		...
		FROM
			...
			LEFT JOIN A5CMUAT.ISSUANCE_CASH_CARD@A5CMUAT_ ICC ON ICC.CSH_BCR_ID = ICCR.CCR_NUMBER
		    LEFT JOIN A5CMUAT.ISSUANCE_CASH_CARD_ACC_MAPPING@A5CMUAT_ ICCM ON ICCM.CAM_CSH_ID = ICCR.CCR_CSH_ID
		    LEFT JOIN A5CMUAT.ISSUANCE_CASH_CARD_ACCOUNT@A5CMUAT_ ICCA ON ICCA.CAC_ID = ICCM.CAM_CAC_ID
	...
	UNION ALL
		SELECT
				...
				''CASH'' AS CARD_TYPE,
				TO_CHAR(CAC.CAC_UPDATED_TS, ''YYYY-MM-DD HH24:MI:SS'') AS ISSUE_DATE,
				CAC.CAC_ACCOUNT_NUMBER,
				INS.INS_CODE AS INSTITUTION_ID,
				CAC.CAC_KEY_ROTATION_NUMBER ROTATION_NUMBER,
				STF1.STF_LOGIN_NAME As Maker,
				STF2.STF_LOGIN_NAME As Checker,
			...
			FROM
				{DCMS_Schema}.ISSUANCE_CASH_CARD_ACCOUNT_REQUEST CAC
	      LEFT JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD_ACC_MAPPING CAM ON CAM.CAM_CAC_ID = CAC.CAC_ID
	      LEFT JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD ICC ON ICC.CSH_ID = CAM.CAM_CSH_ID
				JOIN {DCMS_Schema}.MASTER_STATUS MS ON CAC.CAC_STS_ID=MS.STS_ID
				JOIN {DCMS_Schema}.MASTER_INSTITUTIONS INS ON CAC.CAC_INS_ID = INS.INS_ID
				JOIN {DCMS_Schema}.USER_STAFF STF1 ON STF1.STF_ID = CAC_CREATED_BY
	      JOIN {DCMS_Schema}.USER_STAFF STF2 ON STF2.STF_ID = CAC_UPDATED_BY
			Where
	...
	UNION ALL
		...
		FROM
			DCMS_OWNER.ISSUANCE_CASH_CARD_BALANCE CCB
      LEFT JOIN DCMS_OWNER.ISSUANCE_CASH_CARD_ACC_MAPPING CAM ON CAM.CAM_ID = CCB.CCB_CAM_ID
      LEFT JOIN DCMS_OWNER.ISSUANCE_CASH_CARD ICC ON ICC.CSH_ID = CAM.CAM_CSH_ID
			JOIN DCMS_OWNER.MASTER_STATUS MS ON CCB.CCB_STS_ID = MS.STS_ID
			JOIN DCMS_OWNER.MASTER_INSTITUTIONS INS ON CCB.CCB_INS_ID = INS.INS_ID
			JOIN DCMS_OWNER.USER_STAFF STF1 ON STF1.STF_ID = CCB_CREATED_BY
      JOIN DCMS_OWNER.USER_STAFF STF2 ON STF2.STF_ID = CCB_UPDATED_BY
		Where
