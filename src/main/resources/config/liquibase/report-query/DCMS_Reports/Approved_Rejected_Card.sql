...
-- CBS header/body/trailer fields
		...
		SELECT
			...
			FROM
				{DCMS_Schema}.ISSUANCE_CASH_CARD_ACCOUNT_REQUEST@{DB_LINK_DCMS} CAC
	      		JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD_ACCOUNT@{DB_LINK_DCMS} CCA ON CCA.CAC_ACCOUNT_NUMBER = CAC.CAC_ACCOUNT_NUMBER
	...
	UNION ALL
		SELECT
			...
			''CASH'' AS CARD_TYPE,
      		TO_CHAR(TO_TIMESTAMP(JSON_VALUE(CCB.CCB_AUDIT_LOG, ''$.date''), ''DD-MM-YY HH24:MI:SS''), ''MM/DD/YY HH24:MI'') As ISSUE_DATE,
			CCB.CCB_NUMBER,
			INS.INS_CODE AS INSTITUTION_ID,
...
--      		''UPDATE CC BAL'' AS FUNCTION_NAME,
--			''CASH'' AS CARD_TYPE,
--		    TO_CHAR(To_Date(date_, ''DD-MM-YY HH24:MI:SS''), ''MM/DD/YY HH24:MI'') As ISSUE_DATE,
--			CCB.CCB_NUMBER,
--			INS.INS_CODE AS INSTITUTION_ID,
--			ICC.CSH_KEY_ROTATION_NUMBER,
--			STF1.STF_LOGIN_NAME As Maker,
...
--		    '''' TO_DATA,
--			CASE WHEN CCB.CCB_STS_ID = 68 THEN ''A'' WHEN CCB.CCB_STS_ID = 69 THEN ''R'' ELSE Ms.Sts_Name END As Status
--		FROM {DCMS_Schema}.ISSUANCE_CASH_CARD_BALANCE@{DB_LINK_DCMS} CCB 
--		       CROSS JOIN
--		       JSON_TABLE(
--		         CCB.CCB_AUDIT_LOG,
--		         ''$[*]''
--		         COLUMNS
--		           date_ varchar PATH ''$.date'',
--		           description varchar PATH ''$.description'',
--		           username varchar PATH ''$.username''
--		       )
--		      JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD_ACC_MAPPING@{DB_LINK_DCMS} CAM ON CAM.CAM_ID = CCB.CCB_CAM_ID
--		      JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD@{DB_LINK_DCMS} ICC ON ICC.CSH_ID = CAM.CAM_CSH_ID
--			  JOIN {DCMS_Schema}.MASTER_STATUS@{DB_LINK_DCMS} MS ON CCB.CCB_STS_ID = MS.STS_ID
--			  JOIN {DCMS_Schema}.MASTER_INSTITUTIONS@{DB_LINK_DCMS} INS ON CCB.CCB_INS_ID = INS.INS_ID
--			  JOIN {DCMS_Schema}.USER_STAFF@{DB_LINK_DCMS} STF1 ON STF1.STF_ID = CCB.CCB_CREATED_BY
--		WHERE To_Date(CCB.CCB_UPDATED_TS, ''YYYY-MM-DD HH24:MI:SS'') > To_Date({From_Date}, ''YYYY-MM-DD HH24:MI:SS'') AND
--		    CCB.CCB_INS_ID = 1
--		    AND (description LIKE ''%APPROVED and SUCCESSFULL'' OR description LIKE ''%REJECTED'') 
	UNION ALL
		SELECT
			...
			CRD.CRD_CARDHOLDER_NAME AS CLIENT_NAME,
			'''' FROM_DATA,
      		'''' TO_DATA,
		...
		FROM
			{DCMS_Schema}.ISSUANCE_CARD_STATUS_MAINTENANCE@{DB_LINK_DCMS} CSM
      		JOIN {DCMS_Schema}.ISSUANCE_CARD@{DB_LINK_DCMS} CRD ON CRD.CRD_ID = CSM.CSM_CRD_ID
			JOIN {DCMS_Schema}.MASTER_STATUS@{DB_LINK_DCMS} MS ON CSM.CSM_REQ_STS_ID = MS.STS_ID
	...
	UNION ALL
			SELECT 
	     	''MODIFY CIF'' AS FUNCTION_NAME,
		  	''DEBIT'' AS CARD_TYPE,
	      	TO_CHAR(To_Date(MOC_UPDATED_TS, ''YYYY-MM-DD HH24:MI:SS''), ''MM/DD/YY HH24:MI'') As ISSUE_DATE,
		  	MOC.MOC_REQUEST_NUMBER,
		  	INS.INS_CODE AS INSTITUTION_ID,
			Ic.CRD_KEY_ROTATION_NUMBER,
			STF1.STF_LOGIN_NAME As Maker,
			STF2.STF_LOGIN_NAME As Checker,
			MOC.MOC_REMARKS AS REMARKS,
			IC.Crd_Cardholder_Name AS CLIENT_NAME,
			'''' FROM_DATA,
	      	'''' TO_DATA,
		...
		FROM 
			{DCMS_Schema}.ISSUANCE_MODIFY_CIF@{DB_LINK_DCMS} MOC 
	  			Join {DCMS_Schema}.Issuance_Client_Card_Mapping@{DB_LINK_DCMS} Iccm On MOC.MOC_CLT_ID = Iccm.Ccm_CLT_Id
				Join {DCMS_Schema}.Issuance_Card@{DB_LINK_DCMS} Ic On Iccm.Ccm_Crd_Id = Ic.Crd_Id
