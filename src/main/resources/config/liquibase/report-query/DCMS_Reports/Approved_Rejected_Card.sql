...
-- CBS header/body/trailer fields
	...
	i_BODY_QUERY_2 := TO_CLOB('Union All
		...
		SELECT DISTINCT
			...
			CASE WHEN IDCR.DCR_STS_ID IN (68,69) THEN IDCR.DCR_NUMBER ELSE get_accno(IDCR.DCR_CIF_RESPONSE) END AS ACC_NO,
			INS.INS_CODE AS INSTITUTION_ID,
			IC.CRD_KEY_ROTATION_NUMBER ROTATION_NUMBER,
			STF1.STF_LOGIN_NAME As Maker,
			STF2.STF_LOGIN_NAME As Checker,
	...
	UNION ALL
		...
		FROM
			...
			JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD@{DB_LINK_DCMS} CSH ON CSH.CSH_ID = CSM.CSM_CRD_ID
