...
-- CBS header/body/trailer fields
	...
	i_BODY_QUERY_2 := TO_CLOB('Union All
		...
		SELECT DISTINCT
			...
			ICC.CSH_CARDHOLDER_NAME AS CLIENT_NAME,
			'''' FROM_DATA,
			COALESCE(ICC.CSH_CARD_NUMBER, ICCCRD.CSH_CARD_NUMBER) TO_DATA,
		...
		FROM
			...
			LEFT JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD@{DB_LINK_DCMS} ICCCRD ON ICCCRD.CSH_ID = ICCR.CCR_CSH_ID 
		...
		SELECT DISTINCT
			...
			CASE WHEN IBCR.BCR_STS_ID IN (68,70,165) THEN TO_CHAR(IBCR.BCR_APPROVED_TS, ''MM/DD/YY HH24:MI'')
