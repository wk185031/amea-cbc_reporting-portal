...
-- CBS header/body/trailer fields
	...
	i_BODY_QUERY_2 := TO_CLOB('Union All
		...
		SELECT DISTINCT
			...
			JSON_VALUE(IDCR.DCR_CIF_RESPONSE, ''$.bankAccountCollection.bacAccountNumber'') AS ACC_NO,
			INS.INS_CODE AS INSTITUTION_ID,
			IC.CRD_KEY_ROTATION_NUMBER ROTATION_NUMBER,
			STF1.STF_LOGIN_NAME As Maker,
			STF2.STF_LOGIN_NAME As Checker,
			...
			'''' FROM_DATA,
			'''' TO_DATA,
			CASE WHEN IDCR.DCR_STS_ID = 70 THEN ''A'' WHEN IDCR.DCR_STS_ID = 69 THEN ''R'' ELSE Ms.Sts_Name END As Status
		FROM
			{DCMS_Schema}.ISSUANCE_DEBIT_CARD_REQUEST@{DB_LINK_DCMS} IDCR
			JOIN {DCMS_Schema}.ISSUANCE_CARD@{DB_LINK_DCMS} IC ON IC.CRD_BCR_ID = IDCR.DCR_NUMBER
		...
		Where
			(IDCR.DCR_STS_ID = 70 AND IDCR.DCR_APPROVED_TS >= To_Timestamp({From_Date}, ''YYYY-MM-DD HH24:MI:SS'') And IDCR.DCR_APPROVED_TS <= To_Timestamp({To_Date}, ''YYYY-MM-DD HH24:MI:SS'')
		...
		SELECT DISTINCT
			''NEW CARD'' AS FUNCTION_NAME,
			''CASH'' AS CARD_TYPE,
			CASE WHEN STS_ID = 70 THEN TO_CHAR(ICCR.CCR_APPROVED_TS, ''MM/DD/YY HH24:MI'')
		    ELSE TO_CHAR(ICCR.CCR_UPDATED_TS, ''MM/DD/YY HH24:MI'') END AS ISSUE_DATE,
			JSON_VALUE(ICCR.CCR_CIF_RESPONSE, ''$.bankAccountCollection.bacAccountNumber'') AS ACC_NO,
			INS.INS_CODE AS INSTITUTION_ID,
			ICC.CSH_KEY_ROTATION_NUMBER ROTATION_NUMBER,
			STF1.STF_LOGIN_NAME As Maker,
			STF2.STF_LOGIN_NAME As Checker,
			...
			'''' FROM_DATA,
			'''' TO_DATA,
			CASE WHEN ICCR.CCR_STS_ID = 70 THEN ''A'' WHEN ICCR.CCR_STS_ID = 69 THEN ''R'' ELSE Ms.Sts_Name END As Status
		FROM
			{DCMS_Schema}.ISSUANCE_CASH_CARD_REQUEST@{DB_LINK_DCMS} ICCR 
			JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD@{DB_LINK_DCMS} ICC ON ICC.CSH_BCR_ID = ICCR.CCR_NUMBER
		...
		Where
		    (ICCR.CCR_STS_ID = 70 AND ICCR.CCR_APPROVED_TS >= To_Timestamp({From_Date}, ''YYYY-MM-DD HH24:MI:SS'') And ICCR.CCR_APPROVED_TS <= To_Timestamp({To_Date}, ''YYYY-MM-DD HH24:MI:SS'')
		...
		SELECT DISTINCT
			''NEW CARD'' AS FUNCTION_NAME,
		  	''DEBIT'' AS CARD_TYPE,
			CASE WHEN STS_ID = 70 THEN TO_CHAR(IBCR.BCR_APPROVED_TS, ''MM/DD/YY HH24:MI'')
			...
			'''' FROM_DATA,
			'''' TO_DATA,
			CASE WHEN IBCR.BCR_STS_ID = 70 THEN ''A'' WHEN IBCR.BCR_STS_ID = 69 THEN ''R'' ELSE Ms.Sts_Name END As Status
		FROM
			{DCMS_Schema}.ISSUANCE_BULK_CARD_REQUEST@{DB_LINK_DCMS} IBCR
			JOIN {DCMS_Schema}.ISSUANCE_CARD@{DB_LINK_DCMS} IC ON IC.CRD_BCR_ID = IBCR.BCR_NUMBER
		...
		Where
			(IBCR.BCR_STS_ID = 70 AND IBCR.BCR_APPROVED_TS >= To_Timestamp({From_Date}, ''YYYY-MM-DD HH24:MI:SS'') And IBCR.BCR_APPROVED_TS <= To_Timestamp({To_Date}, ''YYYY-MM-DD HH24:MI:SS'')
		...
		FROM
			{DCMS_Schema}.ISSUANCE_BULK_CARD_REQUEST@{DB_LINK_DCMS} IBCR
			JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD@{DB_LINK_DCMS} ICC ON ICC.CSH_BCR_ID = IBCR.BCR_NUMBER
			JOIN {DCMS_Schema}.MASTER_STATUS@{DB_LINK_DCMS} MS ON IBCR.BCR_STS_ID = MS.STS_ID
			JOIN {DCMS_Schema}.MASTER_INSTITUTIONS@{DB_LINK_DCMS} INS ON IBCR.BCR_INS_ID = INS.INS_ID
			JOIN {DCMS_Schema}.USER_STAFF@{DB_LINK_DCMS} STF1 ON STF1.STF_ID = IBCR.BCR_CREATED_BY
		    JOIN {DCMS_Schema}.USER_STAFF@{DB_LINK_DCMS} STF2 ON STF2.STF_ID = IBCR.BCR_UPDATED_BY
