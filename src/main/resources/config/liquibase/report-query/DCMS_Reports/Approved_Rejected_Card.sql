...
-- CBS header/body/trailer fields
	...
	i_BODY_QUERY_2 := TO_CLOB('Union All
		...
		SELECT DISTINCT
			...
			CASE WHEN IDCR.DCR_STS_ID IN (68,69) THEN IDCR.DCR_NUMBER ELSE JSON_VALUE(IDCR.DCR_CIF_RESPONSE, ''$.bankAccountCollection[0].bacAccountNumber'') END AS ACC_NO,
			INS.INS_CODE AS INSTITUTION_ID,
			IC.CRD_KEY_ROTATION_NUMBER ROTATION_NUMBER,
			STF1.STF_LOGIN_NAME As Maker,
			STF2.STF_LOGIN_NAME As Checker,
	...
	UNION ALL
		SELECT
			...
			''CASH'' AS CARD_TYPE,
      		TO_CHAR(CCB.CCB_UPDATED_TS, ''MM/DD/YY HH24:MI'') As ISSUE_DATE,
			CCB.CCB_NUMBER,
			INS.INS_CODE AS INSTITUTION_ID,
