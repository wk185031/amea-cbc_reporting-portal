...
-- CBS header/body/trailer fields
	...
	select 
  		SUM(NEW_CARD_COUNT + RENEW_COUNT) AS NEW_COUNT,
  		SUM(BULK_UPLOAD_COUNT) AS BULK_UPLOADED_COUNT,
  		SUM(REPLACE_COUNT) AS REPLACEMENT_COUNT,
  		SUM(PREGEN_COUNT) AS PREGEN_COUNT,
  		--SUM(REPIN_COUNT) AS REPIN_COUNT,
  		SUM(NEW_CARD_COUNT + RENEW_COUNT + BULK_UPLOAD_COUNT + REPLACE_COUNT + PREGEN_COUNT) AS TOTAL_COUNT
from (
-- ATM Manual
select 
