...
SELECT DISTINCT
      ...
      CASE 
		WHEN TXN.TRL_ISS_NAME = {V_Iss_Name} AND CBA_ACQ.CBA_MNEM != {V_Acq_Name} THEN CBA_ACQ.CBA_MNEM
		WHEN CBA.CBA_MNEM IS NOT NULL THEN CBA.CBA_MNEM 
		ELSE '''' END AS "BANK MNEM",
      CASE 
...
TXN.TRL_TQU_ID = ''F''
	 ...
	 AND (CBA_ACQ.CBA_MNEM = {V_Acq_Name} OR TXN.TRL_ISS_NAME = {V_Iss_Name})
      AND {Txn_Date}
)
GROUP BY
      "DATE",
