...
UNION ALL
  ...
  Ic.CRD_NUMBER_ENC AS "ATM CARD NUMBER",
	...
	JSON_TABLE((SELECT CRD_AUDIT_LOG FROM DCMS_ISSUANCE_CARD where CRD_ID = ic.Crd_Id),''$[*]'' COLUMNS 
        AUDIT_LOG_DATE VARCHAR2(25) PATH ''$.date'',
...
--CASH CARD UPDATE STATUS
  ...
  ICC.CSH_CARD_NUMBER_ENC AS "ATM CARD NUMBER",
	...
	'''' AS "JOINT ACCOUNT HOLDER TYPE",
	'''' AS "JOINT ACC OPERATION FLAG"
	FROM {DCMS_Schema}.ISSUANCE_CASH_CARD_ACCOUNT_REQUEST@{DB_LINK_DCMS} REQ
	JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD_ACCOUNT@{DB_LINK_DCMS} CCA ON REQ.CAC_ACCOUNT_NUMBER = CCA.CAC_ACCOUNT_NUMBER
	JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD_ACC_MAPPING@{DB_LINK_DCMS} ICCAM On CCA.CAC_ID = ICCAM.CAM_CAC_ID
	JOIN {DCMS_Schema}.ISSUANCE_CASH_CARD@{DB_LINK_DCMS} ICC ON ICCAM.CAM_CSH_ID = ICC.CSH_ID
	join {DCMS_Schema}.ISSUANCE_CLIENT@{DB_LINK_DCMS} CLT on ICCAM.CAM_CLT_ID = CLT.CLT_ID
	JOIN {DCMS_Schema}.MASTER_INSTITUTIONS@{DB_LINK_DCMS} INS ON ICC.CSH_INS_ID = INS.INS_ID
	left join {DCMS_Schema}.CARD_PROGRAM_SETUP@{DB_LINK_DCMS} PRS on PRS.PRS_ID = ICC.CSH_PRS_ID
...
--CARD RENEWAL
  ...
  Ic.CRD_NUMBER_ENC AS "ATM CARD NUMBER",
	...
	'''' AS "JOINT ACCOUNT HOLDER TYPE",
	'''' AS "JOINT ACC OPERATION FLAG"
	from {DCMS_Schema}.Support_Card_Renewal@{DB_LINK_DCMS} Scr
	...
	select CLT.CLT_CIF_NUMBER AS "CIF",
  ''{Iss_Name}'' AS "BANK ID",
  IcNew.CRD_NUMBER_ENC AS "ATM CARD NUMBER",
	IcNew.CRD_KEY_ROTATION_NUMBER "ROTATION_NUMBER",
	INS.INS_CODE AS INSTITUTION_ID,
	BAC.BAC_ACCOUNT_NUMBER AS "FROM ACCOUNT NO",
	''A'' AS "OPERATION FLAG",
	IcNew.CRD_EMBOSSING_NAME AS "ACCOUNT NAME",
	''ATM'' AS "CARD TYPE",
	PRS.PRS_CODE AS "CARD SUBTYPE",
	STS.STS_NAME AS "CARD STATUS",
	'''' AS "NO OF JOINT ACC HOLDERS",
	...
	'''' AS "JOINT ACCOUNT HOLDER TYPE",
	'''' AS "JOINT ACC OPERATION FLAG"
	FROM {DCMS_Schema}.Support_CC_Renewal@{DB_LINK_DCMS} Sccr
