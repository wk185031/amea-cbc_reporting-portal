...
BEGIN 
      ...
      CASE WHEN TXN.TRL_TSC_CODE = 44 AND TXN.TRL_TQU_ID = ''F'' THEN ''IBF''
		   ...
		   WHEN TXN.TRL_TSC_CODE IN (1, 128, 142, 143) AND TXN.TRL_TQU_ID = ''F'' THEN ''CWD''
...
WHERE
	  ...
	  AND (TXN.TRL_TSC_CODE IN (142, 143) OR (TXN.TRL_TSC_CODE = 1 AND TXN.TRL_FRD_REV_INST_ID IS NULL AND TXNC.TRL_ORIGIN_CHANNEL NOT IN (''EBK'', ''MBK'')))
