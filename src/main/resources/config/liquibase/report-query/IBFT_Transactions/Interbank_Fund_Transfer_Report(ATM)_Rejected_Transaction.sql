...
-- IBFT					07-AUG-2021		NY		Empty CBS header field
...
-- Interbank Fund Transfer Report (ATM) Rejected Transaction
...
START ACQUIRING
SELECT
      SUBSTR(TXN.TRL_CARD_ACPT_TERMINAL_IDENT,0,4) "BRANCH CODE",
...
SELECT
      ...
      (SELECT CBA_MNEM FROM CBC_BANK WHERE LPAD(TXN.TRL_FRD_REV_INST_ID, 10, ''0'') = LPAD(CBA_CODE, 10, ''0'')) AS "RECEIVING BANK MNEM",
      SUBSTR(TXN.TRL_ACQR_INST_ID, -4) AS "BANK CODE",
...
END RECEIVING
	...
	WHERE RED_NAME = 'Interbank Fund Transfer Report (ATM) Rejected Transaction' AND RED_INS_ID = 22;
	...
	UPDATE REPORT_DEFINITION SET 
		...
		RED_BODY_QUERY = i_BODY_QUERY,
		RED_TRAILER_QUERY = i_TRAILER_QUERY
	WHERE RED_NAME = 'Interbank Fund Transfer Report (ATM) Rejected Transaction' AND RED_INS_ID = 2;
