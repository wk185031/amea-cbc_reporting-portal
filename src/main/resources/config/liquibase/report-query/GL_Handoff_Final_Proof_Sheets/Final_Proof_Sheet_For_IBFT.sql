...
SELECT DISTINCT TXN.TRL_ID,
      ...
      CASE WHEN GLE.GLE_DEBIT_DESCRIPTION = ''BANCNET SERVICE CHARGE'' THEN 25.00 ELSE TXN.TRL_AMT_TXN END AS "DEBITS",
      0 AS "CREDITS"
FROM
      TRANSACTION_LOG TXN
...
SELECT DISTINCT TXN.TRL_ID,
      ...
      GLA.GLA_NAME "GL ACCOUNT NAME",
      0 AS "DEBITS",
      CASE WHEN GLE.GLE_CREDIT_DESCRIPTION = ''BANCNET SERVICE CHARGE'' THEN 25.00 ELSE TXN.TRL_AMT_TXN END AS "CREDITS"
FROM
      TRANSACTION_LOG TXN
	  LEFT JOIN TRANSACTION_LOG_CUSTOM TLC ON TXN.TRL_ID = TLC.TRL_ID
      LEFT JOIN CBC_GL_ENTRY GLE ON TXN.TRL_TSC_CODE = GLE.GLE_TRAN_TYPE
...
END		
	...
	WHERE RED_NAME = 'Final Proof Sheet For IBFT' AND RED_INS_ID = 22;
	...
	UPDATE REPORT_DEFINITION SET 
		...
		RED_BODY_QUERY = i_BODY_QUERY,
		RED_TRAILER_QUERY = i_TRAILER_QUERY
	WHERE RED_NAME = 'Final Proof Sheet For IBFT' AND RED_INS_ID = 2;
