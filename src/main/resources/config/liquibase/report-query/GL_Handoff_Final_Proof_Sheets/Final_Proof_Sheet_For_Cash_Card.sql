...
SELECT
	...
	FROM
      ...
      JOIN CBC_GL_ENTRY GLE ON GLE.GLE_TRAN_TYPE =
        CASE WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BNT'' and LPAD(txn.trl_frd_rev_inst_id,10,0) = ''0000008882'' THEN 52 
        WHEN TXNC.TRL_ORIGIN_CHANNEL = ''BNT'' and TXN.TRL_FRD_REV_INST_ID IS NOT NULL THEN 44
        ELSE TXN.TRL_TSC_CODE END 
		 ...
		 AND GLE.GLE_MAIN_DIRECTION = CASE WHEN TXN.TRL_TSC_CODE = 48 THEN ''INTER-ENTITY''
           WHEN TXN.TRL_TSC_CODE >= 40 and TXN.TRL_TSC_CODE <= 49 AND TXN.TRL_ISS_NAME = ''CBC'' THEN ''TRANSMITTING'' 
	...
	WHERE
	  ...
	  -- AND TXNC.TRL_ORIGIN_CHANNEL IN (''ATM'',''CDM'',''BRM'',''BNT'')
...
SELECT
 ...
 SELECT
      ...
      GLA.GLA_NUMBER "GL ACCOUNT NUMBER",
      GLA.GLA_NAME "GL ACCOUNT NAME",
      NVL(TXN.TRL_ISS_CHARGE_AMT,0) + NVL(TXN.TRL_ACQ_CHARGE_AMT,0) "DEBIT",
      0 AS "CREDIT"
	FROM
      TRANSACTION_LOG TXN
      JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID = TXNC.TRL_ID
...
SELECT
	 GLE.GLE_DEBIT_DESCRIPTION AS "GROUP_ID",
      ...
      GLA.GLA_NAME "GL ACCOUNT NAME",
      0 AS "DEBIT",
      NVL(TXN.TRL_ISS_CHARGE_AMT,0) + NVL(TXN.TRL_ACQ_CHARGE_AMT,0) "CREDIT"
	FROM
      TRANSACTION_LOG TXN
      JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID = TXNC.TRL_ID
	  LEFT JOIN (SELECT DISTINCT CBL_CODE,CBL_MNEM,CBL_SETTLEMENT_TYPE FROM CBC_BILLER) CBL ON LPAD(TXNC.TRL_BILLER_CODE, 3, ''0'') = LPAD(CBL.CBL_CODE, 3, ''0'')
...
SELECT
	...
	WHERE
	  ...
	  AND TXNC.TRL_ORIGIN_CHANNEL IN (''BNT'',''PGW'')
      AND NVL(CPD.CPD_CODE,0) IN (''80'',''81'',''82'',''83'')
      AND TXN.TRL_ACTION_RESPONSE_CODE = 0
      AND NVL(TXN.TRL_POST_COMPLETION_CODE, ''O'') != ''R''
      AND GLA.GLA_INSTITUTION = {V_Gla_Inst}
