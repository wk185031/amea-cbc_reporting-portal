...
SELECT
      ...
      ''D'' AS "DEBIT CREDIT",
      CASE WHEN LENGTH(GLA.GLA_NUMBER) > 10 THEN SUBSTR(GLA.GLA_NUMBER, 1, 4) ELSE SUBSTR(TXN.TRL_CARD_ACPT_TERMINAL_IDENT, 1, 4) END "BRANCH CODE",
	...
	FROM
      ...
      JOIN CBC_GL_ENTRY GLE ON TXN.TRL_TSC_CODE = GLE.GLE_TRAN_TYPE 
		 ...
		 AND GLE.GLE_MAIN_DIRECTION = CASE WHEN TXN.TRL_TSC_CODE IN (40,42,48) AND TXN.TRL_ISS_NAME = {V_IE_Iss_Name} THEN ''-''
		   WHEN TXN.TRL_TSC_CODE IN (40,42,48) AND LPAD(TXN.TRL_FRD_REV_INST_ID, 10, 0) = {V_IE_Acqr_Inst_Id} THEN ''INTER-ENTITY''
...
SELECT
 ...
 SELECT
      ...
      GLA.GLA_NUMBER "GL ACCOUNT NUMBER",
      GLA.GLA_NAME "GL ACCOUNT NAME",
      SUM(TXN.TRL_ISS_CHARGE_AMT) "DEBIT",
      0 AS "CREDIT"
	FROM
      TRANSACTION_LOG TXN
      JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID = TXNC.TRL_ID
...
SELECT
	 GLE.GLE_DEBIT_DESCRIPTION AS "GROUP_ID",
      ...
      GLA.GLA_NAME "GL ACCOUNT NAME",
      0 AS "DEBIT",
      SUM(TXN.TRL_ISS_CHARGE_AMT) "CREDIT"
	FROM
      TRANSACTION_LOG TXN
      JOIN TRANSACTION_LOG_CUSTOM TXNC ON TXN.TRL_ID = TXNC.TRL_ID
      LEFT JOIN (SELECT DISTINCT CBL_CODE,CBL_MNEM,CBL_SETTLEMENT_TYPE FROM CBC_BILLER) CBL ON LPAD(TXNC.TRL_BILLER_CODE, 3, ''0'') = LPAD(CBL.CBL_CODE, 3, ''0'')
