...
union all select 
	...
	from 
	  ...
	  LEFT JOIN CBC_BIN CBI ON TXNC.TRL_CARD_BIN = CBI.CBI_BIN
	  LEFT JOIN CBC_BANK CBA ON CBI.CBI_CBA_ID = CBA.CBA_ID
	  JOIN ACCOUNT ACN on ACN.ACN_ACCOUNT_NUMBER = (CASE WHEN TXN.TRL_TSC_CODE = 21 THEN TXN.TRL_ACCOUNT_1_ACN_ID ELSE TXN.TRL_ACCOUNT_2_ACN_ID END)
	...
	where 
      ...
      AND TXN.TRL_ACTION_RESPONSE_CODE = 0
	  AND AST.AST_TERMINAL_TYPE = ''BRM''
	  AND (TXN.TRL_ISS_NAME = {V_Iss_Name} OR LPAD(TXN.TRL_FRD_REV_INST_ID, 10, 0) = {V_Acqr_Inst_Id})
      AND TXN.TRL_DEO_NAME = {V_Deo_Name}
      AND ABR.ABR_CODE = {BRANCH_CODE}
	  AND {Txn_Date}
	)
