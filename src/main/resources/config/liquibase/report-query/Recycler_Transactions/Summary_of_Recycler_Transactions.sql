...
select 
	  ...
	  "TRANSACTION GROUP",
	  "TRANSACTION TYPE",
	  SUM(CASE WHEN TRL_ISS_NAME = {V_Iss_Name} AND (TRL_FRD_REV_INST_ID IS NULL OR LPAD(TRL_FRD_REV_INST_ID, 10, 0) = {V_Acqr_Inst_Id}) AND CPD_CODE NOT IN (''81'',''83'') THEN NVL(TRL_AMT_TXN,0) ELSE 0 END) AS "ONUS",
	  SUM(CASE WHEN (TRL_ISS_NAME = {V_IE_Iss_Name} OR LPAD(TRL_FRD_REV_INST_ID, 10, 0) = {V_IE_Acqr_Inst_Id}) AND CPD_CODE NOT IN (''81'',''83'') THEN NVL(TRL_AMT_TXN,0) ELSE 0 END) AS "INTER ENTITY",
	  SUM(CASE WHEN TRL_ISS_NAME IS NULL THEN NVL(TRL_AMT_TXN,0) ELSE 0 END) AS "OTHER BANKS",
	  SUM(CASE WHEN CPD_CODE IN (''81'',''83'') THEN NVL(TRL_AMT_TXN,0) ELSE 0 END) AS "CASH CARD",
	  SUM(CASE WHEN TRL_TSC_CODE IN (1,128,142,143) THEN NVL(TRL_AMT_TXN,0) ELSE 0 END) AS "TOTAL DISPENSED",
	  SUM(CASE WHEN TRL_TSC_CODE = 26 THEN NVL(TRL_AMT_TXN,0) ELSE 0 END) AS "TOTAL DEPOSIT"
	FROM (
    select (ABR.ABR_CODE || '' '' || ABR.ABR_NAME) "BRANCH",
      ...
      CPD.CPD_CODE,
      CASE WHEN TXN.TRL_TSC_CODE = 122 AND TXN.TRL_ACTION_RESPONSE_CODE = 0 AND TXN.TRL_TQU_ID = ''F'' THEN NVL(TXN.TRL_ISS_CHARGE_AMT, 0) 
	    WHEN TXN.TRL_TSC_CODE = 122 AND TXN.TRL_TQU_ID = ''R'' THEN (TXN.TRL_ISS_CHARGE_AMT * -1)
	    WHEN TXN.TRL_TQU_ID = ''R'' THEN (TXN.TRL_AMT_TXN * -1) 
	    ELSE NVL(TXN.TRL_AMT_TXN, 0) END "TRL_AMT_TXN",
      TXN.TRL_ISS_NAME,
      TXN.TRL_TSC_CODE,
	...
	from 
	  ...
	  LEFT JOIN CBC_TRAN_CODE CTR ON TXN.TRL_TSC_CODE = CTR.CTR_CODE 
	    AND CTR.CTR_CHANNEL = CASE WHEN (TXN.TRL_ISS_NAME = {V_IE_Iss_Name} OR LPAD(TXN.TRL_FRD_REV_INST_ID, 10, 0) = {V_IE_Acqr_Inst_Id}) THEN ''I-CDM''  WHEN (TXN.TRL_ISS_NAME = {V_Iss_Name} OR TXN.TRL_TSC_CODE=44) THEN ''CDM'' ELSE ''BNT'' END
	    AND CTR_DEBIT_CREDIT = CASE WHEN TXN.TRL_TSC_CODE = 26 THEN ''CREDIT'' ELSE ''DEBIT'' END 
