...
public class UserService {
	...
	public UserService(UserRepository userRepository, PasswordEncoder passwordEncoder,
			...
			UserExtraSearchRepository userExtraSearchRepository, RoleExtraRepository roleExtraRepository) {
	...
	public User registerUser(UserExtraDTO userExtraDTO, String password) {

		User newUser = new User();
		Authority authority = authorityRepository.findOne(AuthoritiesConstants.USER);
		Set<Authority> authorities = new HashSet<>();
		String encryptedPassword = passwordEncoder.encode(password);
		newUser.setLogin(userExtraDTO.getUser().getLogin());
		// new user gets initially a generated password
		newUser.setPassword(encryptedPassword);
		newUser.setFirstName(userExtraDTO.getUser().getFirstName());
		newUser.setLastName(userExtraDTO.getUser().getLastName());
		newUser.setEmail(userExtraDTO.getUser().getEmail());
		newUser.setImageUrl(userExtraDTO.getUser().getImageUrl());
		newUser.setLangKey(userExtraDTO.getUser().getLangKey());
		// new user is not active
		newUser.setActivated(false);
		// new user gets registration key
		newUser.setActivationKey(RandomUtil.generateActivationKey());
		...
		newUserExtra.setName(userExtraDTO.getUser().getLogin());
		newUserExtra.setUser(newUser);
		Set<RoleExtra> roleExtras = new HashSet<>();
		RoleExtra roleExtra = roleExtraRepository.findOneWithEagerRelationships(PermissionConstants.USER);
		roleExtras.add(roleExtra);
		newUserExtra.setRoles(roleExtras);
	...
	public void updateUserDormantStatus() {
		...
		if (config != null) {
			...
			for (UserExtra userExtra : userExtraList) {
				...
				if (userExtra.getLastLoginTs().toInstant()
						.isBefore(Instant.now().minus(dormantConfig, ChronoUnit.DAYS))) {
	...
	public void updateUserPasswordExpiry() {
		...
		if (config != null) {
			...
			for (User user : userList) {
				...
				Instant passwordResetDate = user.getResetDate() != null ? user.getResetDate() : user.getCreatedDate();
