...
public class GLHandoffBlocksheetIssuer extends TxtReportProcessor {
	...
	protected void writeBody(ReportGenerationMgr rgm, HashMap<String, ReportGenerationFields> fieldsMap)
		...
		for (ReportGenerationFields field : fields) {
			...
			if (!firstRecord && !svc && !bancnet) {
				...
				switch (field.getFieldName()) {
				case ReportConstants.GL_ACCOUNT_NAME:
					if (getFieldValue(field, fieldsMap).contains("SVC")) {
						line.append(String.format("%1$4s", "") + String.format("%1$-" + field.getCsvTxtLength() + "s",
								getFieldValue(field, fieldsMap).substring(0, 25)));
						svcValue = getFieldValue(field, fieldsMap).substring(26,
	...
	private boolean executeQuery(ReportGenerationMgr rgm) {
		logger.debug("In GLHandoffBlocksheetIssuer.executeQuery()");
		ResultSet rs = null;
		PreparedStatement ps = null;
		String query = getBodyQuery(rgm);
