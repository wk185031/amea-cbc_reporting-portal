...
public class GLHandoffBlocksheetBeep extends TxtReportProcessor {
	...
	private void processingDetail(ReportGenerationMgr rgm) {
		logger.debug("In GLHandoffBlocksheetBeep.processingDetail()");
		try {
	...
	private void separateQuery(ReportGenerationMgr rgm) {
		...
		if (rgm.getBodyQuery() != null) {
			...
			setCriteriaQuery(getDebitBodyQuery().replace("TXN.TRL_DEST_STAN \"CODE\",", "")
					...
					.replace(
							"CASE WHEN GLE.GLE_DEBIT_DESCRIPTION IN ('CBC BEEP LOADING') THEN TXN.TRL_AMT_TXN ELSE NVL(TXN.TRL_ISS_CHARGE_AMT, 0) END AS \"DEBIT\",",
							"")
	...
	private boolean executeQuery(ReportGenerationMgr rgm) {
		logger.debug("In GLHandoffBlocksheetBeep.executeQuery()");
		ResultSet rs = null;
		PreparedStatement ps = null;
		String query = getBodyQuery(rgm);
