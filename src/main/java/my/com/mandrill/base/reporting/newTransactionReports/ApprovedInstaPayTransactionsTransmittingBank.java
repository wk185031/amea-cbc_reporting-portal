...
public class ApprovedInstaPayTransactionsTransmittingBank extends IbftReportProcessor {

	private final Logger logger = LoggerFactory.getLogger(ApprovedInstaPayTransactionsTransmittingBank.class);
	...
	private void processingDetails(ReportGenerationMgr rgm, String channel, String bankCode, String bankName) {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.processingDetails()");
		try {
			StringBuilder line = new StringBuilder();
	...
	private void bankSummaryDetails(ReportGenerationMgr rgm) {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.bankSummaryDetails()");
		try {
			StringBuilder line = new StringBuilder();
	...
	private void channelSummaryDetails(ReportGenerationMgr rgm) {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.channelSummaryDetails()");
		try {
			StringBuilder line = new StringBuilder();
	...
	private void separateQuery(ReportGenerationMgr rgm) {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.separateQuery()");
	...
	private void preProcessing(ReportGenerationMgr rgm, String filterByChannel, String filterByBankCode)
		...
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.preProcessing()");
	...
	protected void writeHeader(ReportGenerationMgr rgm, int pagination) throws IOException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.writeRetailHeader()");
	...
	protected void writeSummaryHeader(ReportGenerationMgr rgm, int pagination) throws IOException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.writeSummaryHeader()");
		List<ReportGenerationFields> fields = extractHeaderFields(rgm);
		StringBuilder line = new StringBuilder();
		for (ReportGenerationFields field : fields) {
			switch (field.getSequence()) {
	...
	protected void writeBodyHeader(ReportGenerationMgr rgm) throws IOException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.writeBodyHeader()");
	...
	private void writeSummaryBodyHeader(ReportGenerationMgr rgm, boolean indicator) throws IOException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.writeSummaryBodyHeader()");
		List<ReportGenerationFields> fields = extractBodyHeaderFields(rgm);
		StringBuilder line = new StringBuilder();
		for (ReportGenerationFields field : fields) {
			switch (field.getSequence()) {
	...
	protected void writeTrailer(ReportGenerationMgr rgm, HashMap<String, ReportGenerationFields> fieldsMap)
		...
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.writeTrailer()");
	...
	protected void writeSummaryTrailer(ReportGenerationMgr rgm, HashMap<String, ReportGenerationFields> fieldsMap)
			throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.writeSummaryTrailer()");
		List<ReportGenerationFields> fields = extractTrailerFields(rgm);
		StringBuilder line = new StringBuilder();
		for (ReportGenerationFields field : fields) {
			switch (field.getSequence()) {
	...
	private void executeBodyQuery(ReportGenerationMgr rgm, boolean summary, boolean indicator) {
		logger.debug("In ApprovedInstaPayTransactionsTransmittingBank.executeBodyQuery()");
		ResultSet rs = null;
