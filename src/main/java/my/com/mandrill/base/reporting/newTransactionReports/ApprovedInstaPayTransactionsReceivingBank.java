...
public class ApprovedInstaPayTransactionsReceivingBank extends IbftReportProcessor {
	...
	private void processingDetails(ReportGenerationMgr rgm, String bankCode, String bankName) {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.processingDetails()");
		try {
	...
	private void processingSummaryDetails(ReportGenerationMgr rgm) {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.processingSummaryDetails()");
		try {
	...
	private void separateQuery(ReportGenerationMgr rgm) {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.separateQuery()");
	...
	private void preProcessing(ReportGenerationMgr rgm, String filterByBankCode)
		...
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.preProcessing()");
	...
	protected void writeBodyHeader(ReportGenerationMgr rgm) throws IOException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.writeBodyHeader()");
	...
	private void writeSummaryBodyHeader(ReportGenerationMgr rgm) throws IOException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.writeSummaryBodyHeader()");
		List<ReportGenerationFields> fields = extractBodyHeaderFields(rgm);
		StringBuilder line = new StringBuilder();
		for (ReportGenerationFields field : fields) {
			switch (field.getSequence()) {
	...
	protected void writeTrailer(ReportGenerationMgr rgm, HashMap<String, ReportGenerationFields> fieldsMap)
		...
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.writeTrailer()");
	...
	protected void writeSummaryTrailer(ReportGenerationMgr rgm, HashMap<String, ReportGenerationFields> fieldsMap)
			throws IOException, InstantiationException, IllegalAccessException, ClassNotFoundException, JSONException {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.writeSummaryTrailer()");
		List<ReportGenerationFields> fields = extractTrailerFields(rgm);
		StringBuilder line = new StringBuilder();
		for (ReportGenerationFields field : fields) {
			switch (field.getSequence()) {
	...
	protected void executeBodyQuery(ReportGenerationMgr rgm, boolean summary) {
		logger.debug("In ApprovedInstaPayTransactionsReceivingBank.executeBodyQuery()");
		ResultSet rs = null;
